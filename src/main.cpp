/***************************************************************************
 *  CÓDIGO BUENO PARA ESP32:
 *  Comunicación serie entre ESP32 y ESP8266
 *  Conexión con TTN
 *  Encuentra correctamente los prefijos
 *  Guarda correctamente los payload en los arrays adecuados
 *  Transforma correctamente los valores signed (queda ajustar los unsigned)
 *  Hace la media, max y min de todos (excepto accel, gyro y gps)
 *  
 *  Revisar: no hace del todo bien la media, parece que coge un valor de más
 ***************************************************************************/
...
void read_serial(){
  if(Serial2.available()) {
    ...
    Serial.print("/n Longitud de raw_in_bytes: ");
...
void loop()
{
  ...
  Serial.print("raw_in_bytes: ");
  for(int h = 0; h < sizeof(raw_in_bytes); h++) {
    Serial.print(raw_in_bytes[h], DEC);
    Serial.print(" ");}  
  Serial.println(" ");

  Serial.print("raw_in_char: "); 
  for(int h = 0; h < sizeof(raw_in_char); h++) {
    Serial.print(raw_in_char[h], HEX);
    Serial.print(" ");}  
  Serial.println(" ");

  Serial.print("Valor de cont_bytes antes de save_data: ");
  Serial.println(cont_bytes);
  save_data();
  Serial.print("Valor de cont_bytes después de save_data: ");
  Serial.println(cont_bytes);

  Serial.print("sensor_type: ");
  for(int h = 0; h < sizeof(sensor_type); h++) {
    Serial.print(sensor_type[h]);
    Serial.print(" ");}
  Serial.println(" ");
